...
`D` = terminal's **actual** default background (the JSON's virtual semantic token `background`); `U=userMessageBg`, `C=customMessageBg`, `P=toolPendingBg`, `G=toolSuccessBg`, `R=toolErrorBg`, `S=selectedBg`, `Q=searchMatchBg`; `T={P,G,R}`. The TUI does not supply D. On the Pi branch every text color is a token; text Pi previously left uncolored (terminal default) uses `text`. Validate real colors after quantization and ANSI inverse. Text inside a colored box must be checked on that box even when its foreground color is set elsewhere.

## Inventory and foreground/background pairs

Pi's theme schema and class define **56 semantic slots: 49 foreground + 7 backgrounds**. The structured contract adds the virtual `background` token. Every token has at least one real minimum; there are no `contrast: null` rules. Optional fallbacks: `scrollbarTrack→muted`, `scrollbarThumb→text`, `searchMatchBg→selectedBg`, `searchMatchText→text`, `thinkingMax→thinkingXhigh`. Sources: `theme/theme-json.ts`, `theme/theme.ts`, first-party interactive components, `core/tools/renderers/`, and `packages/tui/src/components/markdown.ts`.
